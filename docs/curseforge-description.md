...
Open **`/fconfig`** or **`/fsmc`** to get started.
...
Create up to **20 named profiles**, inspect saved macros, and choose which sections to restore. **Export** produces text you can keep in a file; **Import** adds a profile without applying it. A **recovery snapshot** is created before each restore, and a report identifies skipped settings or errors.
