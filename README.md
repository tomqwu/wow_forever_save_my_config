...
Open **`/fconfig`** or **`/fsmc`** to save named profiles, inspect their contents, restore selected sections, and copy/paste exports. Version **0.1.2** is an initial preview: tested with mocked client APIs and synthetic Windows backup data, not yet validated inside a running game.
...
4. Click **Export**, **Select all**, then Ctrl+C. Store the entire `FSMC1:...` text in a file outside WoW. Click **Reload** to flush the saved profile database to disk.
5. On the destination, click **Import**, paste, and **Import as new**. This creates a profile without changing settings. Inspect **Macros** and the source character, choose sections, then **Review restore → Apply selected**.
